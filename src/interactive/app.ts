...
import type { Station, Corridor, DataSource, HeadlineStat } from "../types/index.ts";
...
if (container) {
  const svgEl = renderMap(container, { stations, corridors });
  const svg = d3.select(svgEl);
  renderLegend(svg, corridors, headlines);
  bindTooltips(svgEl, stations, corridors, sources);
